class FileFilter:
    ...
    def should_process_file(self, file_name):
        ...
        if file_name in self.exclude_file_names:
            return False
        if any(file_name.endswith(f".{ext}") for ext in self.exclude_extensions):
            return False
        if not self.file_extensions or any(file_name.endswith(f".{ext}") for ext in self.file_extensions):
            return True
        return False
